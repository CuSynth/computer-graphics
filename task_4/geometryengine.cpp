  ...
  for (auto face = 0U; face < 6U; ++face) {
    ...
    for (auto y = 0U; y < factor + 1; ++y) {
      for (auto x = 0U; x < 2; ++x) {
        ...
        position[coord1] = (x * 2.0F - 1.0F) * half_width;
        position[coord2] =
    ...
    // Add indices
    const auto faceIndexOffset = face * (factor + 1) * 2;
    for (auto y = 0U; y < factor; ++y) {
      const auto rowIndexOffset = 2 * y;

      indexes.append(faceIndexOffset + rowIndexOffset);
      for (auto x = 0U; x < 2; ++x) {
        indexes.append(faceIndexOffset + rowIndexOffset + x);
        indexes.append(faceIndexOffset + rowIndexOffset + x + 2);
      }
      indexes.append(faceIndexOffset + rowIndexOffset + 3);
    }
  }
