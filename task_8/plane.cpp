...
plane::plane(const QColor& color, const size_t count, const QVector3D& position, const QVector3D& rotation, float rotation_angle, const QVector3D& scale, bool _raw)
...
void plane::set_subgrid_count(const size_t subgrid_count)
{
...
void plane::set_rotation(float angle, const QVector3D& rotation)
{
    angle_ = angle;
    rotation_ = rotation;
}
