...
void geo_sphere::initialize(QOpenGLShaderProgram* program)
{
	...
	delete texture_night_;
    texture_night_ = new QOpenGLTexture{ QImage(":/Materials/Earth8k_night.jpg") };
	texture_night_->setMinificationFilter(QOpenGLTexture::LinearMipMapLinear);
	texture_night_->setMagnificationFilter(QOpenGLTexture::Linear);
...
void geo_sphere::render()
{
	...
	program_->setUniformValue("uniform_texture_night", 1);
	program_->setUniformValue("uniform_normal_texture", 2);
...
void geo_sphere::set_position(const QVector3D& position)
{
	position_ = position;
}

void geo_sphere::set_rotation(float angle, const QVector3D& direction)
{
	rotation_ = direction;
	angle_ = angle;
}

void geo_sphere::set_subgrid_count(size_t x, size_t y)
{
	subdivision_x_ = x;
	subdivision_y_ = y;
...
void geo_sphere::set_subgrid_count_x(size_t size)
{
	subdivision_x_ = size;
	set_subgrid_count(subdivision_x_, subdivision_y_);
}

void geo_sphere::set_subgrid_count_y(size_t size)
{
	subdivision_y_ = size;
	set_subgrid_count(subdivision_x_, subdivision_y_);
}

geo_sphere::geo_sphere(const size_t subgrid_count_x, const size_t subgrid_count_y, const QColor& color, const QVector3D& position,
