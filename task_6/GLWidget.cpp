...
//	uniform_morph_value_ = program_->uniformLocation("morph_value");
    ...
    my_sphere_.initialize(program_);
//	my_cube_.initialize(program_);
}
...
//	program_->setUniformValue(uniform_morph_value_, morph_value_);
	//ambient light
    ...
    my_sphere_.set_rotation(2 * current_time / screen()->refreshRate(), { 0, 0, 1 });
    my_sphere_.render();
	...
	painter.drawText(rect(), Qt::AlignRight, "triangles: " + QString::number(count_ * count_ * current_subdivision_x_ * current_subdivision_y_ * 12));
	update();
}
...
void GLWidget::set_obj_count(const int n)
{
	count_ = n;
}

void GLWidget::set_morphing_value(const float value)
{
	morph_value_ = value;
}

void GLWidget::set_subdivision_x(const int size)
{
	current_subdivision_x_ = size;
	my_sphere_.set_subgrid_count_x(current_subdivision_x_);
}

void GLWidget::set_subdivision_y(const int size)
{
	current_subdivision_y_ = size;
	my_sphere_.set_subgrid_count_y(current_subdivision_y_);
}
